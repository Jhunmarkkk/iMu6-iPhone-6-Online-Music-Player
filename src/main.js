...
function isControlTarget(element) { while (element && element !== document.body) { if (element.className && String(element.className).indexOf('now-playing') !== -1) return element.id === 'play' || element.id === 'progress' || String(element.className).indexOf('player-control') !== -1 || String(element.className).indexOf('play-button') !== -1; element = element.parentNode } return false }
