...
function render(view, query) {
  ...
  var visible = tracks
  if (view === 'search') {
    visible = query ? (remoteTracks.length ? remoteTracks : tracks.filter(function (track) { return (track.title + track.artist + track.genre).toLowerCase().indexOf(query.toLowerCase()) !== -1 })) : tracks.slice(0, 3)
    content.innerHTML = '<div class="search-wrap"><input id="search-input" type="search" placeholder="Search free music" value="' + escapeHtml(query || '') + '"><span>&#8981;</span></div><div class="section-heading"><h3>Find your next song</h3><span id="search-status">' + (query ? visible.length + ' results' : 'Community audio') + '</span></div><div id="search-results">' + (visible.length ? visible.map(trackRow).join('') : '<p class="empty">Try an artist, title, or mood.</p>') + '</div><p class="catalog-note">Search results come from Internet Archive community audio. Tap a result to stream it, then check its source license.</p>'
  }
...
function bindContent(view, query) {
  ...
  if (input) input.addEventListener('input', function () {
    ...
    searchTimer = setTimeout(function () { remoteTracks = []; render('search', value); if (value.length > 1) searchArchive(value) }, 250)
  })
}
function requestJson(url, done) { var request = new XMLHttpRequest(); request.open('GET', url, true); request.onreadystatechange = function () { if (request.readyState === 4) done(request.status === 200 ? JSON.parse(request.responseText) : null) }; request.send() }
function searchArchive(query) {
  var status = document.querySelector('#search-status')
  if (status) status.textContent = 'searching...'
  var search = 'mediatype:audio AND collection:opensource_audio AND (' + query + ')'
  requestJson('https://archive.org/advancedsearch.php?q=' + encodeURIComponent(search) + '&fl[]=identifier,title,creator,licenseurl&rows=5&page=1&output=json', function (result) {
    var docs = result && result.response ? result.response.docs : []
    if (!docs.length) { if (status) status.textContent = 'no results'; return }
    var pending = docs.length
    remoteTracks = []
    docs.forEach(function (doc, index) {
      requestJson('https://archive.org/metadata/' + encodeURIComponent(doc.identifier), function (metadata) {
        var file = metadata && metadata.files ? metadata.files.filter(function (item) { return item.format === 'VBR MP3' || item.format === 'MP3' })[0] : null
        if (file) remoteTracks.push({ id: 'archive-' + index, title: doc.title || doc.identifier, artist: doc.creator || 'Community audio', genre: 'Community audio', color: ['#d6e6e1', '#f2d8a7', '#e6c6b8', '#cbd5e7'][index % 4], url: 'https://archive.org/download/' + encodeURIComponent(doc.identifier) + '/' + encodeURIComponent(file.name), source: 'https://archive.org/details/' + encodeURIComponent(doc.identifier) })
        pending -= 1
        if (!pending) { var results = document.querySelector('#search-results'); if (results) results.innerHTML = remoteTracks.length ? remoteTracks.map(trackRow).join('') : '<p class="empty">No playable MP3s found.</p>'; if (status) status.textContent = remoteTracks.length + ' results'; bindContent('search', query) }
      })
    })
  })
}
