...
nowSheet.innerHTML = '<div class="sheet-backdrop" id="sheet-backdrop"></div><div class="sheet-panel"><div class="sheet-top"><span class="eyebrow">PLAYING NOW</span><button id="close-sheet" class="sheet-close" aria-label="Close now playing">&times;</button></div><div id="sheet-art" class="sheet-circle-art"></div><div class="sheet-meta"><h2 id="sheet-title"></h2><p id="sheet-artist"></p></div><div class="sheet-progress"><span id="sheet-current-time">0:00</span><input id="sheet-progress" class="sheet-slider" type="range" min="0" max="100" value="0" aria-label="Track progress"><span id="sheet-duration">0:00</span></div><div class="sheet-controls"><button id="sheet-previous" class="sheet-ctrl" aria-label="Previous song">&#9664;&#9664;</button><button id="sheet-play" class="sheet-play" aria-label="Play or pause"></button><button id="sheet-next" class="sheet-ctrl" aria-label="Next song">&#9654;&#9654;</button></div><div class="sheet-modes"><button id="sheet-shuffle" class="sheet-mode-btn" aria-label="Shuffle"><span class="control-icon shuffle-icon"></span></button><button id="sheet-repeat" class="sheet-mode-btn" aria-label="Repeat"><span class="control-icon repeat-icon"></span></button></div></div>'
document.body.appendChild(nowSheet)

function openNowPlaying() { var track = currentTrack(); nowSheet.className = 'now-sheet open'; document.querySelector('#sheet-art').className = 'sheet-circle-art' + (track.thumbnail ? ' has-image' : ''); document.querySelector('#sheet-art').innerHTML = track.thumbnail ? '' : art(track, false); if (track.thumbnail) { document.querySelector('#sheet-art').style.cssText = 'background-image:url("' + escapeHtml(track.thumbnail) + '");background-color:' + track.color } else { document.querySelector('#sheet-art').style.cssText = 'background-color:' + track.color }; document.querySelector('#sheet-title').textContent = track.title; document.querySelector('#sheet-artist').textContent = track.artist; updateSheetControls() }
function closeNowPlaying() { nowSheet.className = 'now-sheet' }
...
function updateSheetControls() { var sheetPlay = document.querySelector('#sheet-play'); if (sheetPlay) sheetPlay.innerHTML = '<span class="control-icon ' + (isPlaying ? 'pause-icon' : 'play-icon') + '"></span>'; var sheetShuffle = document.querySelector('#sheet-shuffle'); var sheetRepeat = document.querySelector('#sheet-repeat'); if (sheetShuffle) sheetShuffle.className = 'sheet-mode-btn' + (shuffleOn ? ' selected' : ''); if (sheetRepeat) sheetRepeat.className = 'sheet-mode-btn' + (repeatOn ? ' selected' : ''); var track = currentTrack(); if (!track) return; var current = 0; var duration = 0; if (track.videoId && ytPlayer && ytPlayer.getCurrentTime) { current = ytPlayer.getCurrentTime() || 0; duration = ytPlayer.getDuration() || 0 } else if (!track.videoId) { current = audio.currentTime || 0; duration = audio.duration || 0 } var currentLabel = document.querySelector('#sheet-current-time'); var durationLabel = document.querySelector('#sheet-duration'); var sheetSlider = document.querySelector('#sheet-progress'); if (currentLabel) currentLabel.textContent = formatTime(current); if (durationLabel) durationLabel.textContent = formatTime(duration); if (sheetSlider) { var pct = duration ? (current / duration * 100) : 0; sheetSlider.value = pct; sheetSlider.style.setProperty('--progress', pct + '%') } }
function isControlTarget(element) { while (element && element !== document.body) { if (element.id === 'play' || element.id === 'progress' || element.id === 'previous' || element.id === 'next') return true; if (element.className && String(element.className).indexOf('player-control') !== -1) return true; if (element.className && String(element.className).indexOf('play-button') !== -1) return true; element = element.parentNode } return false }
...
function updatePlayer() {
  ...
  updateSheetControls()
  if (nowSheet && nowSheet.className.indexOf('open') !== -1) { document.querySelector('#sheet-title').textContent = track.title; document.querySelector('#sheet-artist').textContent = track.artist; var artEl = document.querySelector('#sheet-art'); artEl.className = 'sheet-circle-art' + (track.thumbnail ? ' has-image' : ''); artEl.innerHTML = track.thumbnail ? '' : art(track, false); if (track.thumbnail) { artEl.style.cssText = 'background-image:url("' + escapeHtml(track.thumbnail) + '");background-color:' + track.color } else { artEl.style.cssText = 'background-color:' + track.color } }
}
...
if (shell) { shell.classList.add('shell-enter'); window.setTimeout(function () { shell.classList.remove('shell-enter') }, 600) }
