...
function render(view, query) {
  ...
  var visible = view === 'library' ? tracks : tracks.slice(0, 3)
  if (view === 'search') {
    visible = query ? remoteTracks : tracks.slice(0, 3)
    content.innerHTML = '<div class="search-wrap"><input id="search-input" type="search" placeholder="Search YouTube music" value="' + escapeHtml(query || '') + '"><span>&#8981;</span></div><div class="section-heading"><h3>Find a real song</h3><span id="search-status">' + (query ? 'searching...' : 'YouTube') + '</span></div><div id="search-results">' + (visible.length ? visible.map(trackRow).join('') : '<p class="empty">Search for an artist or song.</p>') + '</div><p class="catalog-note">Results are provided by YouTube. Playback uses the official embedded player.</p>'
  } else if (view === 'library') {
    content.innerHTML = '<div class="section-heading"><h3>Your library</h3><span>demo samples</span></div>' + visible.map(trackRow).join('')
  } else {
...
function selectTrack(id) {
  ...
  if (track.videoId) {
    audio.pause()
    player.innerHTML = '<iframe title="YouTube music player" src="https://www.youtube.com/embed/' + encodeURIComponent(track.videoId) + '?playsinline=1&rel=0&autoplay=1" allow="autoplay; encrypted-media" allowfullscreen></iframe>'
...
function searchYouTube(query) {
  ...
  requestJson('/api/youtube-search?q=' + encodeURIComponent(query), function (result) {
    remoteTracks = result && result.items ? result.items.map(function (item, index) { return { id: 'youtube-' + index, videoId: item.videoId, title: item.title, artist: item.channelTitle, genre: 'YouTube', color: ['#d6e6e1', '#f2d8a7', '#e6c6b8', '#cbd5e7'][index % 4] } }) : []
